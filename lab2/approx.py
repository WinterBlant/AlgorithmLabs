...
for k in range(101):
    ...
    x.append(k/100)
ranges = (slice(0, 1, 0.001), slice(min(y), max(y), 0.001))
result_brute = brute(lst_sq_linear_func, ranges=ranges, args=(x, y), disp=True)
...
result_brute_rational = brute(lst_sq_rational_func, ranges=ranges, args=(x, y), disp=True)
...
for k in range(101):
    y_brute.append(linear(result_brute[0], result_brute[1], x[k]))
    y_powell.append(linear(result_powell[0], result_powell[1], x[k]))
    y_nelder.append(linear(result_nelder[0], result_nelder[1], x[k]))
    y_brute_rational.append(rational(result_brute_rational[0], result_brute_rational[1], x[k]))
